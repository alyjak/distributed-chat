...
use message::{Message, MsgBuilder, MsgClient, MsgCodec, MsgTyp, NACKKind};
...
pub type BuilderAccessor = Arc<Mutex<MsgBuilder>>;
...
fn handle_message(
    msg_or_err: Result<Message, String>,
    client: Bytes,
    addr: SocketAddr,
    peer_map: PeerMapAccessor,
    sock_map: SockMapAccessor,
    ...
    match msg_or_err {
        Ok(msg) => {
            ...
            let mut update_usermap = false;
            {
                ...
                // PeerChannels are always created in main, who also ensures that peer_map has an
                // entry for the addr before initialization of the peerchannel.
        ...
        Err(message) => {
            ...
            tokio::spawn(peer);
            stderr().poll_write(
                format!("Error parsing message from {}. Error: {}", addr, message).as_bytes(),
            )?;
        }
    };
    Ok(())
